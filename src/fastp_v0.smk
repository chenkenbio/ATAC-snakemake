...
rule fastp_pe:
    ...
    output:
        ...
        html = outdir / "reads_trimmed/{sample}/{sample}_fastp.html",
        json = outdir / "reads_trimmed/{sample}/{sample}_fastp.json"
    log:
        outdir / "reads_trimmed/{sample}/{sample}_fastp.log"
    params:
        title = lambda wildcards: f"Fastp report for {wildcards.sample}",
    threads:
        8
    shell:
        """
